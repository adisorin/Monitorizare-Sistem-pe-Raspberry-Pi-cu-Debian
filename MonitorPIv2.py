...
app = ctk.CTk()
app.title("Raspberry Pi System Monitor")
...
# ------------------ UPDATE UI ------------------
# ------------------ UPDATE UI ------------------
def update_ui(cpu, ram, disk, down, up, cpu_text, ram_text, temp):
    cpu_label.configure(text=f"CPU:      {cpu:5.1f} %   Temp: {temp:5.1f} °C")
    ...
    # Adaugăm temperatura în ceas analog, sub CPU
    draw_analog(clock_canvas, 100, 100, 80, cpu, "CPU")
    draw_analog(clock_canvas, 300, 100, 80, ram.percent, "RAM")
    draw_analog(clock_canvas, 500, 100, 80, disk.percent, "SSD")
    draw_analog(clock_canvas, 700, 100, 80, min(down / 1024 / 1024 * 10, 100), "NET")
    draw_analog(clock_canvas, 900, 100, 80, min(temp, 100), "TEMP")  # Temperatura CPU
...
def update_stats():
    ...
    while True:
        ...
        # Preluăm temperatura CPU flexibil
        ...
        app.after(0, update_ui, cpu, ram, disk, down, up, cpu_text, ram_text, temp)
        time.sleep(2)
